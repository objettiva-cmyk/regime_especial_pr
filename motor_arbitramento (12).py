...
# interface externa do script. A versão 1.3.9 consolida ajustes de
# governança, incluindo nomenclatura dinâmica para o arquivo de
# ajustes do analista e correções de metadata.
SCRIPT_VER  = "1.3.9"
    ...
    for d in dirs:
        ...
        for entry in d.iterdir():
            ...
            if entry.suffix.lower() == ".xlsx":
                # Tenta abrir e verificar colunas mínimas
                try:
                    with pd.ExcelFile(entry) as xl:
                        sheet = xl.sheet_names[0] if xl.sheet_names else None
                        if sheet:
                            df_prev = pd.read_excel(entry, sheet_name=sheet, nrows=3, dtype=object, keep_default_na=False)
                            cols_norm = {norm_text(c) for c in df_prev.columns}
                            has_codigo = any(c in cols_norm for c in {"CODIGO", "CODIGOPRODUTO", "CODIGOITEM", "CODIGO_ITEM", "CPROD", "SKU"})
                            has_ncm = any(c in cols_norm for c in {"NCM", "NCMITEM", "CODIGO_NCM", "NCMPRODUTO"})
                            if not has_codigo or not has_ncm:
                                issue_msg = "Colunas obrigatórias ausentes (CODIGO ou NCM)."
                except Exception as exc:
